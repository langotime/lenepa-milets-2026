...
class LabeledMaskCollator:
  """Wrapper for MaskCollator that handles (x, y) sample tuples."""
  def __init__(self, mask_collator):
    self.mask_collator = mask_collator

  def __call__(self, batch):
    xs, ys = zip(*batch)
    x, mask_encoder, mask_predictor = self.mask_collator(xs)
    y = torch.utils.data.default_collate(ys)
    return x, y, mask_encoder, mask_predictor

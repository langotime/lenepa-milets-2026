...
This appendix specifies the fixed-horizon protocol and the main PTB-XL/Diag experiment matrix used in this paper.
...
\paragraph{Datasets.}
We report main-matrix results on (i) Diag, an anonymized synthetic diagnostic \emph{basic-components} benchmark (classification and dense regression targets) introduced in a concurrent anonymized submission, and (ii) PTB-XL~\citep{wagner2020ptbxl} (real ECG; classification).
We use Diag only as diagnostic instrumentation during our experiments, thanks to its configurability and the availability of fine-grained labels for both categorical and dense targets.
...
For PTB-XL and Diag, we train offline linear probes on frozen per-layer representations for layers $0$--$8$ and evaluate them every $1{,}000$ steps (including step $0$).
...
For the PTB-XL/Diag matrix, we repeat each (objective configuration, dataset) pair with five independent random seeds $s\in\{0,1,2,3,4\}$.
...
Diag generation is driven by explicit stream seeds, which are fixed in our configs, so varying $s$ changes optimization/model randomness but not the diagnostic data stream.
...
To disentangle objective-inherent behavior from artifacts of class frequency, we evaluate two versions of the basic-components stream: (i) a \textbf{balanced} sampler with uniform component weights (Figure~\ref{fig:diag_balanced_basic_components_combined_radar_grid}) and (ii) an \textbf{imbalanced} sampler used in our main matrix (Figure~\ref{fig:diag_combined_radar_grid}), where six components are sampled $50\times$ more rarely than the rest.
...
\begin{subfigure}{\textwidth}
  \centering
  \includegraphics[width=\textwidth,height=0.40\textheight,keepaspectratio]{figures/diag_balanced_basic_components_combined_radar_grid.pdf}
  \caption{Balanced sampler (uniform component weights).}
  \label{fig:diag_balanced_basic_components_combined_radar_grid}
\end{subfigure}
...
\caption{Last-step (step $20{,}000$) best-layer frozen-backbone probe deltas relative to step $0$ (untrained initialization) on PTB-XL and Diag. For each method and metric, the reported value is the best score over probed layers $0$--$8$, selected separately at step $0$ and at the last step.}
...
\caption{Absolute last-step (step $20{,}000$) best-layer frozen-backbone probe performance on PTB-XL and Diag. Bars show macro AUROC/AUPRC for both datasets and MSE/MAE/Pearson/$R^2$ for Diag. For each method and metric, the reported value is the best score over probed layers $0$--$8$.}
...
\input{tables/projector_ablation_last_step.tex}

Across the $24$ metric comparisons in Table~\ref{tab:projector_ablation_last_step} (two PTB-XL metrics and six Diag metrics for each of three configurations), projector-enabled variants are better in $22/24$ cases.
...
The top row shows AUROC/AUPRC for PTB-XL and Diag, while the bottom row shows Diag dense-target metrics (MSE/MAE/Pearson/$R^2$).
...
\caption{SIGReg component ablations (seed $0$ only; projector enabled): best-layer transfer dynamics for time-only (\texttt{T20}), batch-only (\texttt{B20}), representation-only (\texttt{R20}), innovation-only (\texttt{I20}), batch-time-only (\texttt{BT20}), and the multi-term reference configuration (\texttt{BRI20}). Top row: classification AUROC/AUPRC for PTB-XL and Diag. Bottom row: Diag dense-target metrics (MSE/MAE/Pearson/$R^2$), where lower is better for MSE/MAE. Best layer is selected independently at each step (over layers $0$--$8$).}
\label{fig:sigreg_component_ablations_dynamics}
\end{figure*}

\subsection{Diag Dense Target Training Dynamics}
\label{sec:appendix_experiments_diag_dense_dynamics}

Figure~\ref{fig:diag_dynamics_best_layer_dense} reports training dynamics for dense regression targets on the Diag \emph{basic-components} benchmark.
For each metric and each offline-probe evaluation step, we plot the best-layer value over probed layers $0$--$8$.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/diag_dynamics_best_layer_dense.pdf}
\caption{Diag \emph{basic-components} dense-target dynamics: best-layer over layers $0$--$8$ frozen-backbone probe performance over training steps for MSE/MAE/Pearson/$R^2$.}
\label{fig:diag_dynamics_best_layer_dense}
\end{figure*}
